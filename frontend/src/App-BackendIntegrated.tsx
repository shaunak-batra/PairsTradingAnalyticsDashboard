...
export default function App() {
      ...
      (data: API.WebSocketMessage) => {
        if (data.type === 'initial') {
          ...
          if (data.prices) {
            ...
            if (data.prices[symA]) setPriceA(data.prices[symA]);
            if (data.prices[symB]) setPriceB(data.prices[symB]);
          }
          ...
          if (data.ohlc) {
            const symA = symbolA.toLowerCase();
            const symB = symbolB.toLowerCase();
            if (data.ohlc[symA]) setOhlcA(data.ohlc[symA]);
            if (data.ohlc[symB]) setOhlcB(data.ohlc[symB]);
          }
          ...
          if (data.volumes) {
            const symA = symbolA.toLowerCase();
            const symB = symbolB.toLowerCase();
            if (data.volumes[symA]) setVolumeA(data.volumes[symA]);
            if (data.volumes[symB]) setVolumeB(data.volumes[symB]);
          }
        }
      },
      (error) => {
  ...
  useEffect(() => {
    if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
      // Just update local state - WebSocket streams all symbols
      console.log(`Symbols updated to ${symbolA} / ${symbolB}`);
    }
  }, [symbolA, symbolB]);
    ...
    <div style={styles.container}>
      ...
      <div style={{ textAlign: 'center', padding: '2rem 0', borderTop: '1px solid #30363d', marginTop: '2rem' }}>
        ...
        <p style={{ color: '#8b949e', fontSize: '0.75rem', marginTop: '0.5rem' }}>
          Phase 2 Complete: Backend Integration · Real-time Binance Data · Statistical Analysis
        </p>
      </div>
    </div>
  );
